...
const Search = () => {
  ...
  const apiLink = !useSearchLink
    ? `https://www.omdbapi.com/?s=Batman&apikey=184ffaab`
...
console.log("Loading", data?.Search)
    
  return (
    <div className='w-full px-[77px] font-dmsans mt-[98px]' >
        <form onSubmit={onSearch} className='w-full '>
            <p className='mb-[4px] text-[16px] md:text-[24px]'>Search</p>
            <input className='outline-none border w-full border-black indent-3 h-[34px] md:h-[54px]' value={searchInput} onChange={(e) => setSearchInput(e.target.value)}/>

        </form>
        {isLoading && <p className='text-center text-lg mt-3'>Searching........</p>}
        {isError && <p className='text-center text-lg mt-3'>Oops.... An Error Occured.....</p>}
        {searchResult && <div className='mt-[48px]  text-[18px] md:text-[24px]'>
        ...
        {!searchResult && <p className='text-center mt-20 text-3xl'>Please Search A Movie!</p>}
    </div>
  )
}
